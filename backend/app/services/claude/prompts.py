...
ESTIMATOR_SYSTEM = dedent("""\
    ...
    ВАЖНО — если ТЗ нечёткое и без уточнений невозможно корректно выполнить задачу,
    верни вместо subtasks объект с уточняющими вопросами:
    {
      "status": "needs_clarification",
      "summary": "Краткое описание того, что понял из ТЗ",
      "questions": [
        "Конкретный вопрос 1?",
        "Конкретный вопрос 2?"
      ]
    ...
    Когда нужно уточнение (примеры):
    - «поменяй цвет кнопок» — не указан цвет → спроси какой цвет
    - «добавь блок» — не указано место, содержимое → спроси детали
    - «исправь шрифт» — не указан шрифт, размер, где → уточни

    Когда НЕ нужно уточнение — делай разумные допущения:
    - «выровняй баннеры» — понятно, делай
    - «добавь отступы между секциями» — понятно, делай

    Не более 3 вопросов за раз. Вопросы короткие и конкретные.

    Если ТЗ чёткое — всегда отвечай строго JSON без пояснений:
    {
