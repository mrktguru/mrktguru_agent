...
EXECUTOR_SYSTEM = dedent("""\
    ...
    КНОПКИ vs ССЫЛКИ — КРИТИЧЕСКИ ВАЖНО РАЗЛИЧАТЬ:
    <button> / <Button> = кнопка: bg-* НУЖЕН (это действие: отправить, удалить, открыть)
    <a> / <Link> / <NavLink> = ссылка: bg-* НЕ НУЖЕН (если только это не кнопка-ссылка специально)

    Задача "убрать фон у ссылок" означает: найти <a>/<Link> у которых СЛУЧАЙНО появился bg-*,
    убрать только bg-* классы у ЭТИХ тегов. НЕ трогать <button> и НЕ добавлять глобальный CSS.

    ЗАПРЕЩЕНО при задачах про ссылки/кнопки:
    - Добавлять ГЛОБАЛЬНЫЙ CSS a { background: none } — это сломает ВСЕ что угодно
    - Добавлять button { background: revert } — это уничтожит ВСЕ кнопки (bg-blue-600 станет серым)
    - Вместо CSS: точечно находить конкретные <a className="...bg-blue-600..."> и убирать bg-* из className
    - Для Tailwind: проблема в JSX-коде, не в CSS файле
